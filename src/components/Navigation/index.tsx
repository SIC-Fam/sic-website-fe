...
const NavList = (props: { list: Array<NavbarListType> }) => {
  const { list } = props;
  return (
    <ul className="my-2 flex flex-col gap-1 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      {list.length > 0 &&
        list.map((_l, _i) =>
          _l.menus ? (
            ...
            <Typography
              ...
              variant="small"
              color="blue-gray"
              className={`p-1 cursor-pointer font-medium py-3 px-8 hover:shadow-glove border-b hover:border-primary transition-colors border-transparent ${style['navbar-name']}`}
            >
              <a
                href={_l.href}
...
const SICNavbar = (props: NavbarType) => {
  ...
  return (
    <Navbar fullWidth color="transparent" shadow className="sticky top z-10 mx-auto shadow-primary">
